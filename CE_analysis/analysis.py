# -*- coding: utf-8 -*-
"""
Created on Fri May  7 16:23:12 2021
...
size = 28
setup = 0
configs = np.zeros((size,30))
configs_dist = np.zeros((size,30))
...
count = 0
for seed in seeds:
    filepath_1 = "testing\seed_"+str(count)+"\seed"+str(seed)+".csv" 
...
for freq in [ 0.1, 0.5, 0.9]:
    for replace_n in [0.1, 0.25, 0.5]:
        ...
        count = 0
        for seed in seeds:
            filepath_1 = "testing\seed_"+str(count)+"\seed"+str(seed)+"freq"+str(freq)+"num_replace"+str(replace_n)+"method"+str(1)+".csv" 
            df_1 = pd.read_csv(filepath_1)
            
            filepath_2 = "testing\seed_"+str(count)+"\seed"+str(seed)+"freq"+str(freq)+"num_replace"+str(replace_n)+"method"+str(2)+".csv" 
            df_2 = pd.read_csv(filepath_2)
            
            filepath_3 = "testing\seed_"+str(count)+"\seed"+str(seed)+"freq"+str(freq)+"num_replace"+str(replace_n)+"method"+str(3)+".csv" 
            ...
            configs_dist[setup][count] = (df_1["Avg Hamming 1"].iat[-1] + df_1["Avg Hamming 2"].iat[-1]) /2
            configs_dist[setup+1][count] = (df_2["Avg Hamming 1"].iat[-1] + df_2["Avg Hamming 2"].iat[-1]) /2
            configs_dist[setup+2][count] = (df_3["Avg Hamming 1"].iat[-1] + df_3["Avg Hamming 2"].iat[-1]) /2
...
configs_2 = []
configs_dist_2 = []
labels = ["method" + str(i+1) for i in range(size)]
...
df["Method"] = [str(i) for i in range(1, size+1)]
